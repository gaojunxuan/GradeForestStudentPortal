...
export default class AcademicsScreen extends React.Component {
  ...
  async componentDidMount() {
    try {
      ...
      if (username === null || password === null) {
        ...
        await this.changeTerm('gtmQ40000000Q4');
  ...
  async changeTerm(term) {
    ...
    let classListGetResponse = await fetch('https://aspen.cps.edu/aspen/portalClassList.do', { method: 'GET', headers: uaHeaders, credentials: 'include' });
    if (classListGetResponse.status !== 200) {
      await Networking.login(username, password);
      classListGetResponse = await fetch('https://aspen.cps.edu/aspen/portalClassList.do', { method: 'GET', headers: uaHeaders, credentials: 'include' });
    }
    ...
    assignmentForm.append('userEvent', 950);
    assignmentForm.append('termFilter', term);
    assignmentForm.append('formFocusField', 'gradeTermOid');

    // get assignments list
    const academicsResponse = await fetch('https://aspen.cps.edu/aspen/portalClassList.do', {
